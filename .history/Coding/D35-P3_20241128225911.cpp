...
void dfs(int person, vector<vector<int>>& relations, vector<bool>& visited, int P) {
    visited[person] = true;
    for (int other = 0; other < P; other++) {
        if (relations[person][other] == 1 && !visited[other]) {
            dfs(other, relations, visited, P);
        }
    }
}

int findRelationChains(vector<vector<int>>& relations, int P) {
    vector<bool> visited(P, false);
    int relationChains = 0;

    for (int i = 0; i < P; i++) {
        if (!visited[i]) {
            // Start a new relation chain
            dfs(i, relations, visited, P);
            relationChains++;
        }
    }
...
int main() {
    int P;
    cin >> P;
    vector<vector<int>> relations(P, vector<int>(P));
    for (int i = 0; i < P; i++) {
        for (int j = 0; j < P; j++) {
            cin >> relations[i][j];
        }
    }
    cout << findRelationChains(relations, P) << endl;
    return 0;
}
